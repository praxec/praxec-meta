...
### Authoring-loop hardening (dogfooding findings #4 + #5)

- **Non-vacuous check gate (finding #4).** The author flows' `checking` state ran `praxec check` before the emitted YAML ever reached disk, so the "it must load" gate validated the UNCHANGED gateway and passed vacuously. Both `flow.author-flow` and `flow.author-capability` now insert `writing` → `written` between `emitting` and `checking`: new cap `cap.install.emitted-definition` (script `install.emitted-definition`, atomic temp + same-dir rename) writes the emitted YAML to its `target_path` first, and the `written` gate branches to `failed` on `write_ok == false` so a failed write can never fall through to another vacuous check. This closes the "emit step still does not persist the YAML to disk" note in the 0.0.14-alignment entry below.
- **Bounded review-repair loop (finding #5).** A `needs-revision`/`rejected` review verdict previously routed straight to `failed`, discarding the composed work. `reviewed` now routes non-approved verdicts BACK to `composing`, bounded by a `repair_round` counter against `max_repair_rounds` (default 2, same LoopGuard discipline as `flow.repair-workflow-health`); on exhaustion, explicit `abort_*` transitions route to `failed`. The review's findings ride `$.context.review_findings` into the composer: `cap.plan.compose-implementation` gains an optional `review_findings` input (default `[]`) and its goal requires each revised shape to address every finding. The PR-#17 unguarded `default: → failed` stays as the out-of-domain fallback.
